...
-- 6. tblInsa. 여름태생(7~9월) + 여자 직원 총 몇명?
    ...
    select count(*) from tblInsa where ssn like '%-2%' and
    (ssn like '__07%' or ssn like '__08%' or ssn like '__09%');
...
select round(avg(basicpay + sudang)) from tblInsa where jikwi in ('부장', '과장');

--4. tblInsa. 사원급(대리+사원)의 평균 급여?

select round(avg(basicpay + sudang)) from tblInsa where jikwi in ('대리', '사원');

--5. tblInsa. 장급(부장,과장)의 평균 급여와 사원급(대리,사원)의 평균 급여의 차액?

-- 조건을 만족하면 then 값을 반환한다를 이용
...
avg(case
when 
jikwi in ('부장', '과장') then basicpay
end) -
avg(case
when 
jikwi in ('대리', '사원') then basicpay
end)
from tblInsa;
